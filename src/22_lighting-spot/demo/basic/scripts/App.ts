...
export class App {
  ...
  private readonly limitValue: Float32Array<ArrayBuffer>;
  ...
  private constructor(fields: {
    ...
    limitValue: Float32Array<ArrayBuffer>;
    ...
    this.limitValue = fields.limitValue;
  ...
  static async create(canvas: HTMLCanvasElement): Promise<App> {
    ...
    // normalMatrix(12) + worldViewProjection + world + color
    //   + lightWorldPosition + viewWorldPosition + shininess + lightDirection + limit
    ...
    const limitValue = uniformValues.subarray(kLimitOffset, kLimitOffset + 1);
  ...
  private render() {
    ...
    this.limitValue[0] = Math.cos(this.settings.limit);

    // 平面がないので、スポットライトを F に向ける
    {
      const mat = mat4.aim(
