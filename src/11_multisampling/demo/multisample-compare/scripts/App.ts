...
export class App {
  ...
  private constructor(fields: {
    ...
    this.sampleCount = fields.sampleCount;
  ...
  static async create(
    ...
    const shaderModule = device.createShaderModule({
      label: 'multisample compare triangle',
      code: index_wgsl,
    });
    ...
    device.lost.then(async (info) => {
      ...
      if (info.reason !== 'destroyed') {
        app.dispose();
        const next = await App.create(canvas, sampleCount);
        next.start();
      }
    });
  ...
  private resize(entries: ResizeObserverEntry[]) {
    for (const entry of entries) {
      ...
      const lowWidth = Math.round(width / RESOLUTION_DIVISOR);
      const lowHeight = Math.round(height / RESOLUTION_DIVISOR);

      const maxSize = this.device.limits.maxTextureDimension2D;
      this.canvas.width = Math.max(1, Math.min(lowWidth, maxSize));
      this.canvas.height = Math.max(1, Math.min(lowHeight, maxSize));
    }
    this.render();
  }
