...
export class App {
  ...
  private static readonly kLightDirection = [-0.5, -0.7, -1];
  ...
  static async create(canvas: HTMLCanvasElement): Promise<App> {
    ...
    // world + worldViewProjection + color + lightDirection
  ...
  private render() {
    ...
    mat4.rotationY(this.settings.rotation, this.worldValue);

    mat4.multiply(
      viewProjectionMatrix,
      this.worldValue,
      this.worldViewProjectionValue,
    );

    this.colorValue.set(App.kColor);
    this.lightDirectionValue.set(vec3.normalize(App.kLightDirection));
