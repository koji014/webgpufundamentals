...
export class App {
  ...
  private static readonly numFs = 5 * 5 + 1;
  ...
  private render() {
    ...
    this.settings.target[0] = Math.cos(this.settings.targetAngle) * App.radius;
    this.settings.target[2] = Math.sin(this.settings.targetAngle) * App.radius;
    ...
    this.objectInfos.forEach(
      ({ matrixValue, uniformBuffer, uniformValues, bindGroup }, i) => {
        ...
        if (i < 25) {
          ...
          const aimMatrix = mat4.aim([x, 0, z], this.settings.target, up);
