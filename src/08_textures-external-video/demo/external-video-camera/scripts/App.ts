...
export class App {
  ...
  static async create(canvas: HTMLCanvasElement): Promise<App> {
    ...
    // ファイルを指定せず、クリック時に getUserMedia でカメラをソースにする。
    const video = document.createElement('video');
    await App.waitForClick(video);
  ...
  private static waitForClick(video: HTMLVideoElement): Promise<void> {
    return new Promise((resolve, reject) => {
      window.addEventListener(
        ...
        async () => {
          ...
          try {
            ...
            reject(
              e instanceof Error ? e : new Error('カメラにアクセスできません'),
            );
          }
        },
        { once: true },
