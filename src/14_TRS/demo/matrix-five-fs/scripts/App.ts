...
export class App {
  ...
  static async create(canvas: HTMLCanvasElement): Promise<App> {
    ...
    // オブジェクトごとに uniform バッファとバインドグループを用意する
  ...
  private render() {
    ...
    for (const {
      ...
      resolutionValue.set([this.canvas.width, this.canvas.height]);
      matrixValue.set(mat3ToPadded(matrix));
