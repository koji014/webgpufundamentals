...
const main = async () => {
  ...
  const render = () => {
    ...
    colorAttachment.view = currentTexture.createView(); // createView: テクスチャの一部の範囲だけを切り出す指定ができる（引数なし＝デフォルトの範囲）
    ...
    pass.setPipeline(pipeline); // パイプラインをセット
  ...
  // canvas のリサイズ （簡易版）
  // ResizeObserver: 監視対象の要素の「表示サイズ（CSSピクセル）」が変わるたびにコールバックが呼ばれる
  // __ canvas は「表示サイズ(CSS)」と「描画解像度(canvas.width/height)」が別物。
  // __ 表示サイズの変化に描画解像度を追従させないと、拡大時に描画がぼやける。
  const observer = new ResizeObserver((entries) => {
    ...
    for (const entry of entries) {
      ...
      render(); // リサイズ後のサイズの新たなテクスチャの生成は、render の中に書いた context.getCurrentTexture() が行なう
    }
  });

  // canvas の監視を開始する。
  // __ observe した直後にも一度コールバックが発火するため、これが初回描画の起点にもなる
